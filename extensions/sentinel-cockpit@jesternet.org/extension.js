...
class SentinelCockpit extends PanelMenu.Button {
    ...
    _showZombieDetails() {
        try {
            // Get fresh zombie list from system
            const zombieScript = `
...
echo ''
echo '═══════════════════════════════════════'
echo 'To clean zombies, kill their parent process.'
echo 'Parent PIDs:'
echo ''
...
read
`;
            const command = `${this._terminalCmd} bash -c "${zombieScript}"`;
            GLib.spawn_command_line_async(command);
        } catch (e) {
            log('[Sentinel Cockpit] Failed to show zombie details: ' + e.message);
        }
    }
    ...
    _displayProcessSnapshot(snapshot) {
        ...
        // Zombies - store for click handler
        this._currentZombies = snapshot.zombies || [];
        const zombieCount = this._currentZombies.length;
        const zombieEmoji = zombieCount > 0 ? '🧟' : '✅';
        let zombieItem = new PopupMenu.PopupMenuItem(`${zombieEmoji} Zombies: ${zombieCount}${zombieCount > 0 ? ' (click to view)' : ''}`);
        zombieItem.connect('activate', () => this._showZombieDetails());
